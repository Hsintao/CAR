...
class PyramidAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(len(self.scale)):
            ...
            if self.scale[i] != 1:
                ref = F.interpolate(
                    input, scale_factor=self.scale[i], mode='bicubic')
            ...
            raw_w_i = raw_w_i.view(
                shape_input[0], shape_input[1], kernel, kernel, -1)
            # raw_shape: [N, L, C, k, k]
            ...
            w_i = extract_image_patches(ref_i, ksizes=[self.ksize, self.ksize],
                                        ...
                                        rates=[1, 1],
                                        padding='same')
            w_i = w_i.view(shape_ref[0], shape_ref[1],
        ...
        for idx, xi in enumerate(input_groups):
            # group in a filter
            wi = torch.cat([w[i][idx][0] for i in range(
                len(self.scale))], dim=0)  # [L, C, k, k]
            # normalize
            ...
            raw_wi = torch.cat([raw_w[i][idx][0]
                                for i in range(len(self.scale))], dim=0)
            yi = F.conv_transpose2d(
                yi, raw_wi, stride=self.stride, padding=1) / 4.
            y.append(yi)
